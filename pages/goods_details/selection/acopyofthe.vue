				...
				<view class="popup_content" v-if="getobj.good_type != 2 || getobj.good_type != 4">
					...
					<view class="cont_item" v-if="activeSelect.material">
						...
						<view class="selectAll">
							...
							<div class="select" @click="showSelect1(true,1,getobj.spec.material,'twoSelect')">
...
<script>
	...
	export default {
		...
		methods: {
			...
			loadData() {
				var _this = this;
				this.$u.post(this.api.getRead, {
					id: this.id,
					type: 1
				}).then(res => {
					if (res.code == 1) {
						this.loadDatason(res)
					} else {
						uni.showToast({
							title: res.msg,
							icon: 'none',
			...
			getPrice(type, pay, add) {
				...
				if (type == 1) {
					let data = {
						...
						material_weight_id: _this.twoActive.value == '' ? '' : _this.twoActive.value ? _this.twoActive
							.value : _this.getobj.spec.material[0].specs[0].id, //纸张宽度id
						print_type: '2', //印刷类型
					...
					this.$u.post(this.api.OtherPrice, {
						...
						if (add) {
							console.log(res.data[0].this, '加入购物车')
							let addData = {
								good_id: this.getobj.id,
								spec: '',
						...
						if (res.code == 1) {
							...
							if (pay) {
								...
								data.spec = JSON.stringify(res.data[0].this);
								data.special = JSON.stringify(dataa)
								uni.setStorageSync('payObj', JSON.stringify(data))
								this.$u.post(this.api.buynow, data).then(res => {
									if (res.code == 1) {
										uni.navigateTo({
											url: '/pages/order/submit_orders/index?option=' + JSON
												.stringify(res.data) + '&specId=0',
										})
									}
								})
							}
					...
					_this.$u.post('goods/postAreaCraftsSpec', {
						good_id: this.goodsMessage.good_id,
						good_type: this.goodsMessage.good_type,
						number: _this.activeSelect['numbers'].active ? _this.activeSelect['numbers'].name ? _this
							.activeSelect['numbers'].name : _this.getobj.spec.numbers[0].num : _this.activeSelect[
			...
			pnumbers() {
				...
				if (this.activeSelect['pnumbers']) {
					let pnumberss = _this.activeSelect['pnumbers'].name ? _this.activeSelect['pnumbers'].name.split("P")
