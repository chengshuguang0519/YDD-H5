	...
	<view class="order_details">
		...
		<view class="order_carid_b">
			<view class="order_carid">
				<view class="carid_car">
					...
					<view>
						...
						<view><text>￥{{goodsList.good_price}}</text><text>x{{goodsList.num}}</text></view>
					</view>
				</view>
			...
			<view class="order_list">
				...
				<view><text>商品总价</text><text>￥{{dataList.total_price}}</text></view>
...
<script>
	export default {
		...
		methods: {
			getOrderList(opent){
				this.$u.get(this.api.refundgetRead,{
					id:opent.id
				}).then(res=>{
					this.dataList = res.data[0];
					this.address = res.data[0].address_info;
					this.goodsList = res.data[0].good_snap;
				})
			},
			CountTime(id){
				this.showDay = false
